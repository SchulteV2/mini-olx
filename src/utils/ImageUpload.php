<?php
    class ImageUpload {
        ...
        private function clean($string) {
            $sem_espacos = strtolower(str_replace(' ', '-', $string)); // Substitui todos os espaços por hifen
        
            $string_ok = preg_replace('/[^A-Za-z0-9\-]/', '', $sem_espacos); // remove caracteres especiais(çáà)
        ...
        public function verify() {
            ...
            if(isset($type) && !in_array($type, array("image/png", "image/jpeg", "image/jpg"))) {
                $this->errors[] = "O arquvio enviado não é um arquivo de imagem válido";
                return;
            }
        }
    }
